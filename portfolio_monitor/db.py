...
CREATE TABLE IF NOT EXISTS bigdrop_candidates (
    asof_date TEXT NOT NULL,
    threshold TEXT NOT NULL,
    ticker TEXT NOT NULL,
    name TEXT,
...
def save_bigdrop_candidates(conn, asof_date: str, threshold: str, candidates: list[dict]) -> None:
    ...
    conn.executemany(
        """INSERT OR REPLACE INTO bigdrop_candidates
           (asof_date, threshold, ticker, name, price, year_low, year_high, pct_from_52w_low,
            pct_from_52w_high, pct_change_1d, pct_change_1w, pct_change_1m, target_mean,
            target_upside_pct, analyst_ratings_json, buy_ratio_pct, ratings_count, market_cap,
            is_pre_revenue, is_pre_revenue_reason, trailing_pe)
           VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        [
            (
                asof_date,
...
def get_bigdrop_candidates(conn, asof_date: str, threshold: str) -> list[dict]:
    rows = conn.execute(
        "SELECT * FROM bigdrop_candidates WHERE asof_date=? AND threshold=? ORDER BY pct_change_1d ASC",
        (asof_date, threshold),
    ).fetchall()
    result = []
    for r in rows:
...
def get_latest_bigdrop_candidates_date(conn, threshold: str) -> Optional[str]:
    row = conn.execute(
        "SELECT MAX(asof_date) AS d FROM bigdrop_candidates WHERE threshold=?", (threshold,)
    ).fetchone()
    return row["d"] if row else None
